    ...
    return (
        ...
        <div className="flex-1 overflow-y-auto max-[700px]:scrollbar-none p-4 px-0 space-y-4">
            ...
            {selectedPosts.map((post)=>(
                ...
                <div
                    key={post._id}
                    className={`chat border-t-2 border-gray-500 ${post.postedBy===authUser._id?"chat-end":"chat-start"} min-[700px]:mx-5  `}
                    ref={postTopRef}
                >
                    <div className={`chat-header w-11/12 justify-between ${post.postedBy===authUser._id?"flex-row-reverse":""} max-[750px]:chat-header avatar`}>
                        ...
                        <div className="h-10 w-36 pt-2"><p className="w-max text-blue-600 text-xl">{post.fullName.split(" ")[0]}</p></div>
                    </div>


                    <div className="chat-bubble w-full flex flex-col text-center">
                        <div className="inline-block max-w-full mx-auto overflow-auto ">
                            {post.img && <img
                            src={post.img}
                            alt="Attachment"
                            className="sm:max-w-[500px] min-w-[300px] rounded-md mb-2 object-contain"
                            />}
                        </div>
                        {/* //// Post text //// */}
                        ...
                        <div className={`flex justify-between min-w-48`}>
                            <div className={`flex space-x-4 items-center text-gray-600 ${deletePostId===post._id.toString()?"hidden":""}`}>
                                ...
                                <button onClick={()=>setReplyTo(replyTo===post._id?"":post._id)} className="hover:text-blue-500 text-gray-300 space-x-1">
                                    ...
                                    <MessageCircle className={`w-5 h-5 inline-block ${replyTo===post._id?"text-blue-500":""}`} />
                                </button>
                        ...
                        {replyTo===post._id && <div className="flex-1 flex flex-col overflow-auto">
                            <div className="w-full my-2 overflow-y-auto min-h-0 max-h-28 border-gray-500 border-t-2">
        ...
        {showAllPosts || selectedUser?._id===authUser._id?
            ...
            {addPost?(<PostInput setAddPost={setAddPost}/>):(
                <button
                    onClick={()=>{setAddPost(prev=>!prev)}}
                    className="relative bottom-5 right-10 w-auto h-10 px-2 bg-base-300
                        flex items-center justify-center cursor-pointer rounded-full float-end"
                >
                    <p className="text-4xl pb-1">+</p>
                    <p>New Post</p>
                </button>
            )}
            </div>
